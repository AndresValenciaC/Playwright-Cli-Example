...
import { TEST_URLS, UserType, getUserByType } from "../test-data/test-data-users";
...
export async function authenticateUser(
    ...
    try {
        ...
        throw new Error(
            `Authentication failed for user ${username}: ${error instanceof Error ? error.message : String(error)
            }`,
        );
    }
}
...
export async function verifyAuthState(
    ...
    try {
        ...
        console.log(`✗ Failed to verify auth state: ${error}`);
        return false;
    }
}
...
export function deleteAuthFile(authFile: string): void {
    try {
        ...
        console.warn(`Failed to delete auth file: ${error}`);
    }
}