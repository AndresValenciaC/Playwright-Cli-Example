...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/seed.test.ts --debug=cli
...
**File:** `tests/<group>/<kebab-case-scenario-name>.test.ts`
...
- Scenario names are kebab-case and match the test file name (`should-add-single-todo` → `should-add-single-todo.test.ts`).
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/<group>/<scenario>.test.ts
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright test tests/<group>/<scenario>.test.ts:<line> --debug=cli
